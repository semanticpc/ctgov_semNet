...
from util.mysql_wrapper import add_concepts, add_sentence, add_trial
...
def _worker(process_ids, corpusPath, stopwords, tuis, outPath, pid):
    ...
    for i in xrange(0, len(process_ids)):
        ...
        for sent in sentences:
            ...
            sentID = trial_ID + '-' + str(sentCount)
            sentCount += 1

            # Add Sentence to Database
            try:
                add_sentence(cur, sentID, trial_ID, unicode(sent).encode('ascii','ignore'), 'None')
                db.commit()
            except:
                db.rollback()
            # degub = False
            ...
            for window in tags:
                for concept in window:
                    cui = concept[0]
                    concept_text = concept[1]
                    try:
                        add_concepts(cur, cui, concept_text, sentID, trial_ID)
                        db.commit()
                    except:
                        db.rollback()
...
if __name__ == '__main__':
    ...
    # log.info('processing clinical trials')
    # procs = []
    # _worker([nct_ids[0]], corpus_path, stopwords, tuis, args.out, 1)