...
def filterCUI(in_queue, redis_dict):
    ...
    while True:
        ...
        if lang == 'ENG' and suppress == 'N':
            ...
            redis_dict.sadd(str(source_code_id), ' '.join(text))
            redis_dict.sadd(str(source_code_id), cui)

            redis_dict.sadd(' '.join(text), str(source_code_id))
...
def build_CUI_db(umls_path, tui_map=False):
    ...
    num_workers = 10
    redis_dict = redis.StrictRedis('localhost', port=6380)
    manager = Manager()
...
if __name__ == "__main__":
    umls_path = "/Users/praveen/work/data/umls/2014AA/META/"
    redis_dict = redis.StrictRedis(host='localhost', port=6380)
    redis_dict.flushall()
